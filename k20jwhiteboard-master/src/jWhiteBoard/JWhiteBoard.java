...
public class JWhiteBoard extends ReceiverAdapter implements ActionListener,
	...
	private JButton clearButton, leaveButton, colorbackgroundButton,
			colorbrushButton;
	 ...
	 * int green = Math.abs(random.nextInt()) % 255; int blue =
	 * Math.abs(random.nextInt()) % 255; return new Color(red, green, blue); }
	 **/
	...
	public void viewAccepted(View v) {
		...
		if (v instanceof MergeView) {
			...
			// This is a simple merge function, which fetches the state from the
			// coordinator
					...
					try {
						...
						if (send_own_state_on_merge) {
							synchronized (drawPanel.state) {
								copy = new LinkedHashMap<Point, Color>(
										drawPanel.state);
							}
						}
						...
						if (copy != null)
							sendOwnState(copy); // multicast my own state so
